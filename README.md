...
磁盘不足以保留 30 份数据库副本时，显式启用指标保留模式：

```bash
bash scripts/run_agent_native_ablation.sh full --retention metrics-only
```

该模式在每个物理 variant 完成后，先校验结果并原子写入 `result_checkpoint.json`，再删除该 variant 的 `data/` 和 MinIO S3 prefix，保留 capability、measurement、公共指标、模块原始测量、服务日志、CSV 与 checkpoint。默认 `--retention full` 不删除任何数据库数据。
...
| Storage | 本地 SSD；full retention 默认至少 250 GB；metrics-only full 至少 40 GB且运行时保留 10 GB安全线 | 清理/扩容；不要把 Badger data dir 放在 NFS 上 |
...
全量输入当前约 3.6 GB、36k 文件。共享 Full 使用一个 Badger data directory 和 S3 prefix，其余 29 个物理 variant 各自隔离。`--retention full` 会保留全部副本，预检最低 250 GB，并建议按 smoke/中等运行后的单 variant 占用扩容到 500 GB 以上。`--retention metrics-only` 逐个清理已完成副本，full 预检最低 40 GB，运行期间可用空间低于 10 GB 时会在继续 ingest 前失败退出。
...
代码、数据和 Linux 构建完成后，先运行仓库内的 preflight。`smoke` 检查 10 GB 可用空间；`full` retention 默认要求 250 GB，`full --retention metrics-only` 要求 40 GB。可通过 `PLASMOD_ABLATION_MIN_FREE_GB` 提高门槛。
...
| `--retention full|metrics-only` | 保留全部数据库副本，或逐 variant 仅保留指标、日志和 checkpoint | 磁盘受限的正式 full 使用 `metrics-only`；恢复时保持不变 |
| `--disk-floor-gb N` | metrics-only 运行时的最低剩余磁盘安全线，默认 10 GB | 触线后失败退出，不会自动反复重启 |
| `--skip-build` | 跳过 runner 开始时的 `make build` | 仅限已经按当前 core commit 构建并通过 preflight；不跳过 C++ `.so` 检查 |
| `--resume` | 复用同一 run id 中的 variant checkpoint 和完整分组 CSV | 必须保持 commit、数据、event/query limit、port 和 retention 不变 |
